<#
    ...
    Descripción:
        ...
        - Instala librerías necesarias para UiPath y automatización
...
# Librerías requeridas para UiPath + Playwright + Data Processing
$RequiredPackages = @(
    "playwright",
    "pandas",
    "openpyxl",
...
# ==============================
Write-Log "Verificando librerías necesarias..."
if (Test-Path $PythonExe) {
    ...
    try {
        Write-Log "🌐 Instalando navegadores Playwright..."
        & "$PythonExe" -m playwright install chromium | Out-Null
        Write-Log "✔️ Navegador Chromium instalado correctamente."
    } catch {
        Write-Log "❌ Error al instalar navegadores de Playwright: $_" "ERROR"
    }
}
